...
function ApiBoardPage() {
    ...
    return (
        <div className="board-container">
            <h1 className="board-header">온비드 공매 물건 조회</h1>

            <div className="board-filter-container">
                <label className="board-label">
                    처분방법
                    <select value={prptDvsnCd} onChange={(e) => { setPrptDvsnCd(e.target.value); setCurrentPage(1); }} className="board-select">
                        <option value="0001">매각</option>
                        <option value="0002">임대</option>
                    </select>
                </label>
                <div className="board-filter-group">
                    <label htmlFor="cltrNm" className="board-label">물건명</label>
                    <input type="text" id="cltrNm" value={cltrNm} onChange={(e) => setCltrNm(e.target.value)} placeholder="물건명 검색" className="board-input" />
                </div>
                <div className="board-filter-group">
                    <label htmlFor="sido" className="board-label">시/도</label>
                    <input type="text" id="sido" value={sido} onChange={(e) => setSido(e.target.value)} placeholder="시/도" className="board-input" />
                </div>
                <div className="board-filter-group">
                    <label htmlFor="sgk" className="board-label">시군구</label>
                    <input type="text" id="sgk" value={sgk} onChange={(e) => setSgk(e.target.value)} placeholder="시/군/구" className="board-input" />
                </div>
                <div className="board-filter-group">
                    <label htmlFor="emd" className="board-label">읍면동</label>
                    <input type="text" id="emd" value={emd} onChange={(e) => setEmd(e.target.value)} placeholder="읍/면/동" className="board-input" />
                </div>
                (총 {totalCount}건)
                                ...
                                <tr key={itemIndex}>
                                    {FIELD_DEFINITIONS.map((field, colIndex) => (
                                        <td key={colIndex}>
                                            ...
                                            ) : field.key === "minBidPrc" ? formatCurrency(item[field.key])
            ...
            <Pagination />
            {isModalOpen && <DetailModal isOpen={isModalOpen} onClose={closeDetailModal} item={selectedItem} openPage="apiBoard" onSearch={handleSearchButtonClick}/>}
        </div>
    );
}
